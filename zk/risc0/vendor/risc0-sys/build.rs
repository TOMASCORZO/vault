...
fn main() {
    ...
    // Vault patch: upstream 1.5.0 attempts to build Metal assets on every
    // macOS host, even when the existing `metal` feature is disabled. This
    // machine intentionally uses the CPU prover and does not have Xcode's
    // optional Metal compiler. Keep CPU kernels intact and make the advertised
    // feature control Metal compilation.
