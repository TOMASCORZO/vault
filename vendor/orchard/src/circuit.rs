...
impl Config {
    /// Configures the hardened Action circuit for composition inside a parent
    /// Halo 2 circuit.
    ///
    /// This produces the same columns and constraints as the standalone
    /// [`Circuit`] implementation. The parent must call
    /// [`Circuit::synthesize_for_composition`] with the returned configuration.
    pub fn configure_for_composition(meta: &mut plonk::ConstraintSystem<pallas::Base>) -> Self {
        Self::configure(meta)
    }

    /// Loads the Sinsemilla table shared by every composed Action.
    ///
    /// A parent circuit that synthesizes multiple Actions with one [`Config`]
    /// must call this exactly once before synthesizing those Actions.
    pub fn load_for_composition(
        &self,
        layouter: &mut impl Layouter<pallas::Base>,
    ) -> Result<(), plonk::Error> {
        SinsemillaChip::load(self.sinsemilla_config_1.clone(), layouter)
    }
...
/// Already-constrained private cells returned when an Action is synthesized
/// inside a parent circuit.
///
/// These handles let the parent reuse the exact note values and expanded
/// receivers checked by the Action statement. They do not expose witness
/// values outside the proof.
#[derive(Clone, Debug)]
pub struct ActionCells {
    v_old: AssignedCell<NoteValue, pallas::Base>,
    v_new: AssignedCell<NoteValue, pallas::Base>,
...
impl ActionCells {
    /// Value cell of the note consumed by this Action.
    pub fn old_value(&self) -> &AssignedCell<NoteValue, pallas::Base> {
        &self.v_old
    }

    /// Value cell of the note created by this Action.
    pub fn new_value(&self) -> &AssignedCell<NoteValue, pallas::Base> {
        &self.v_new
    }

    /// Affine coordinate cells of the consumed note's expanded receiver in
    /// `(g_d.x, g_d.y, pk_d.x, pk_d.y)` order.
    pub fn old_address_coordinates(&self) -> [AssignedCell<pallas::Base, pallas::Base>; 4] {
        [
            self.g_d_old.inner().x(),
            self.g_d_old.inner().y(),
            self.pk_d_old.inner().x(),
            self.pk_d_old.inner().y(),
        ]
    }

    /// Affine coordinate cells of the created note's expanded receiver in
    /// `(g_d.x, g_d.y, pk_d.x, pk_d.y)` order.
    pub fn new_address_coordinates(&self) -> [AssignedCell<pallas::Base, pallas::Base>; 4] {
        [
            self.g_d_new.inner().x(),
            self.g_d_new.inner().y(),
            self.pk_d_new.inner().x(),
            self.pk_d_new.inner().y(),
        ]
    }
}
...
impl Circuit {
    ...
    fn synthesize_base(
    ...
    ) -> Result<ActionCells, plonk::Error> {
        ...
        let v_net_magnitude_sign = {
            ...
            layouter.constrain_instance(
                cv_net.inner().x().cell(),
                config.primary,
                instance_offset + CV_NET_X,
            )?;
            layouter.constrain_instance(
                cv_net.inner().y().cell(),
                config.primary,
                instance_offset + CV_NET_Y,
            )?;
        ...
        // Spend authority (https://p.z.cash/ZKS:action-spend-authority)
        {
            ...
            layouter.constrain_instance(
                rk.inner().x().cell(),
                config.primary,
                instance_offset + RK_X,
            )?;
            layouter.constrain_instance(
                rk.inner().y().cell(),
                config.primary,
                instance_offset + RK_Y,
            )?;
        }
            ...
            |mut region| {
                    ...
                    coordinate_checks.into_iter().enumerate()
                {
                    ...
                    let cross_address_disabled = region.assign_advice_from_instance(
                        || "disableCrossAddress",
                        config.primary,
                        instance_offset + DISABLE_CROSS_ADDRESS,
                        config.advices[0],
                        offset,
                    )?;
    ...
    /// Synthesizes this Action inside a parent circuit and returns handles to
    /// the private cells already constrained by the Action statement.
    ///
    /// `instance_offset` is the first row of this Action's canonical ten-row
    /// public instance. Multiple Actions can therefore share one instance
    /// column without sharing public rows. The caller is responsible for
    /// providing the matching canonical instance values at that offset.
    pub fn synthesize_for_composition(
        &self,
        config: &Config,
        layouter: &mut impl Layouter<pallas::Base>,
        instance_offset: usize,
    ) -> Result<ActionCells, plonk::Error> {
        let cells = self.synthesize_base(config, layouter, instance_offset)?;

        if self.circuit_version.supports_cross_address_restriction() {
            Self::synthesize_cross_address_checks(config, layouter, &cells, instance_offset)?;
        }
...
impl plonk::Circuit<pallas::Base> for Circuit {
    ...
    fn synthesize(
        ...
        config.load_for_composition(&mut layouter)?;
        self.synthesize_for_composition(&config, &mut layouter, 0)
            .map(|_| ())
    }
}
...
impl Instance {
    ...
    /// Returns the canonical Halo 2 instance column for this Action.
    ///
    /// Parent circuits concatenate these ten-row values in Action order and
    /// pass the matching row offsets to [`Circuit::synthesize_for_composition`].
    pub fn to_halo2_instance(&self) -> [[vesta::Scalar; 10]; 1] {
